...
public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		DBinsert dbin = (DBinsert) wac.getBean("DBinsert");//插入
		DBupdate dbup = (DBupdate) wac.getBean("DBupdate");//更新
		DBdelete dbde = (DBdelete) wac.getBean("DBdelete");//删除
		String action = request.getParameter("action").trim(); // 得到请求动作
		if (action.equals("login")) { // 请求动作为登陆时
			String aname = request.getParameter("uname").trim();// 得到用户名
			String apwd = request.getParameter("upwd").trim(); // 得到密码
			String hql = "from AdminInfo as p " + "where p.aname='" + aname + "' and p.apwd='" + apwd + "'"; // hql语句
			...
			} else if (type.equals("consumerback")) { // 销售退货
			...
			} else if (type.equals("providerback")) {//采购退货
			...
			if (list.isEmpty()) {
				...
				List goodslist = db.getPageContent(userBean.getHql(), userBean.getNowPage(), userBean.getSpan()); // 得到要显示的内容
