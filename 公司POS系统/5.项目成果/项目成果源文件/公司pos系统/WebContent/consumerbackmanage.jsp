...
<html>
  ...
  <body>
	...
	<table width="100%" height="44" bgcolor="#206AB3">
      <tr align="center"><td><font color="#FFFFFF" size="5">销售退货管理</font></td></tr>
	</table>
	<table>
	  ...
	  <tr>	    
		...
		<table height="42" style="background:url(img/add_sear.jpg) no-repeat">
		  <tr>
		    ...
		    <td>
		      <input name="key" id="key" value="请输入要搜索的表单号" style="border:0" size="28"	onfocus="document.all.key.value=''"/>		    
		    </td>
	...
	<table width="100%" border="0" cellspacing="1" bgcolor="black">
	  ...
	  <% 
	   ...
	   %>
	     ...
	     <td><%= ci.getCname() %></td>
	     <td><%= cb.getEid() %></td>
	     <td><%= (cb.getCbdate().getYear()+1900)+"-"+(cb.getCbdate().getMonth()+1)+"-"+(cb.getCbdate().getDate()) %></td>
	  ...
	  <tr>
	    ...
	    <td align="right">
	      <% 
	       ...
	       %>
	      <a href="ManageServlet?action=changePage&pagename=/consumerbackmanage.jsp&page=<%= userBean.getNowPage()-1 %>" target="mainFrame"><input type="button" name="prev" value="上页"></a>
	      <% 
	      	}
	      	if(userBean.getNowPage()<userBean.getTotalPage()){
	       %>	       
	      <a href="ManageServlet?action=changePage&pagename=/consumerbackmanage.jsp&page=<%= userBean.getNowPage()+1 %>" target="mainFrame"><input type="button" name="next" value="下页"></a>
	      <% 
	      	}
	      	else{
